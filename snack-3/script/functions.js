
// // ###   funzione gara dadi utente computer

// function GetRandomNumber() {
//     const randomNumber = MathFloor(MathFloor() * 6 + 1);
//     return randomNumber;
// }

// const dadoUtente = GetRandomNumber();
// const dadoComputer = GetRandomNumber();

// if(dadoUtente > dadoComputer) {
//     alert("Utente vince")
// } else if(dadoUtente < dadoComputer){
//     alert("Computer vince")
// }else if( dadoUtente == dadoComputer){
//     alert("Pareggio")
// }

// // ### funzione numero Randomico

function getRandomNumber(min , max){
    min = parseInt(min);
    max = parseInt(max);

    if(isNaN(min) || isNaN(max)){
    alert("I valori inseriti devono essere numerici");
    return;
    }

    if(min >= max){
        console.log("il valore massimo deve essere maggiore del valore minimo");
        return;
    }

    const getRandomNumber = (MathFloor(Math.random() * (max - min +1) + min));

    return getRandomNumber;
    
    
}
// const getRandomNumber2 = (from = 1, to = 100) => { return Math. floor(Math. random() * (to - from + 1) + from); };